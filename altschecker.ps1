...
function Start-AutoAnalyze {
    ...
    Update-Status -Text "Analisi $($found.Count) file..." -Color "Success" -Force
    Show-SearchWindow -Files $found
}
...
function Show-SearchWindow {
    ...
    $txtSearch.Location = New-Object System.Drawing.Point(85, 12)
    ...
    $lv.Add_SelectedIndexChanged({
        if ($lv.SelectedIndices.Count -gt 0) {
            ...
            $data = $resultData[$idx]
            $rtb.Clear()
            $header = "File: $($data.File)`r`nRiga: $($data.Line)`r`n" + ("-" * 60) + "`r`n`r`n"
